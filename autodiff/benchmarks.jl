...
function alloc_timer(n;E,K)
    x = rand(n)
    t0 = @belapsed f2(x) evals=E samples=K
    t1 = @belapsed ForwardDiff.gradient(f2,x) evals=E samples=K
    t2 = @belapsed ReverseDiff.gradient(f2,x) evals=E samples=K
    t3 = @belapsed Zygote.gradient(f2,x) evals=E samples=K
    t4 = @belapsed Enzyme.gradient(Enzyme.Forward,f2,x) evals=E samples=K
    t5 = @belapsed Enzyme.gradient(Enzyme.Reverse,f2,x) evals=E samples=K
    t1,t2,t3,t4,t5,t0
end
